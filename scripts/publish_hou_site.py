#!/usr/bin/env python3
"""Upload BHG Site Working source/ to House of Ur Library and ensure Site ``working``.

House: Burra History Group (prod). Does **not** touch Preview or Real of the
public history site.

Credentials (first match):
  env HOU_API_KEY / HOU_API_BASE / HOU_HOUSE_ID
  scripts/.env  (BHG key only — do not reuse Bronze Arch)

Usage:
  python scripts/publish_hou_site.py
  python scripts/publish_hou_site.py --build
"""
...
DEFAULT_HOUSE_ID = "9075a3ed-54e7-4162-8db8-54087cedac59"  # Burra History Group
LIBRARY_PREFIX = "/sites/bhgsite-working"
SITE_SLUG = "working"

# Existing public history Sites — never upload here.
FORBIDDEN_PREFIXES = (
    "/sites/bhgsite2026-preview/",
    "/sites/bhgsite2026/",
    "/sites/bhgsite2026-real/",
)
...
def load_hou_config() -> dict:
    ...
    cfg = {
        ...
        # Always default to BHG unless HOU_HOUSE_ID is set. Do not inherit
        # Bronze Arch's house id from the shared .env.
        "house_id": (os.environ.get("HOU_HOUSE_ID") or DEFAULT_HOUSE_ID).strip(),
        "api_key": (
...
def upload_file(api_base: str, token: str, house_id: str, library_path: str, local: Path) -> None:
    for forbidden in FORBIDDEN_PREFIXES:
        if library_path.startswith(forbidden.rstrip("/")):
            raise RuntimeError(f"Refusing to upload to public site folder: {library_path}")
...
def walk_source_files() -> list[tuple[str, Path]]:
...
def unwrap_site(obj: dict) -> dict:
    if isinstance(obj.get("site"), dict):
        return obj["site"]
    if isinstance(obj.get("data"), dict) and obj["data"].get("site_slug"):
        return obj["data"]
    return obj
...
def ensure_site(api_base: str, token: str, house_id: str) -> dict:
    ...
    for s in sites:
        if str(s.get("site_slug") or "") == SITE_SLUG:
            existing = s
            break
        lib = str(s.get("library_path") or "")
        if lib.rstrip("/") == LIBRARY_PREFIX:
            existing = s
            break
    payload = {
        "library_path": path,
    ...
    if existing:
        ...
        print(f"  site exists {existing.get('site_slug')} id={site_id} status={existing.get('status')}")
        return api_json(
            api_base,
            token,
            "PATCH",
...
def wait_active(api_base: str, token: str, house_id: str, site: dict, *, timeout_s: int = 420) -> dict:
    ...
    while time.time() < deadline:
        ...
        time.sleep(8)
        current = unwrap_site(
            api_json(api_base, token, "GET", f"/api/houses/{house_id}/sites/{site_id}")
        )
...
def public_url(site: dict) -> str:
    ...
    if host:
        return f"https://{host}/"
    return f"https://{SITE_SLUG}-bhg.house-of-ur.com/"


def summarize_sites(api_base: str, token: str, house_id: str) -> None:
    listed = api_json(api_base, token, "GET", f"/api/houses/{house_id}/sites")
    sites = listed.get("data") or listed.get("sites") or []
    if isinstance(listed, list):
        sites = listed
    print(f"existing sites ({len(sites)}):")
    for s in sites:
        slug = s.get("site_slug")
        lib = s.get("library_path")
        host = s.get("hostname") or s.get("host")
        print(f"  {slug}  {lib}  {host}  status={s.get('status')}")


def main() -> int:
    ap = __import__("argparse").ArgumentParser(description=__doc__)
    ap.add_argument("--build", action="store_true", help="Run scripts/build.py first")
    ap.add_argument("--list-only", action="store_true", help="Auth and list sites, then stop")
    args = ap.parse_args()
    if args.build:
        print("=== build ===")
        subprocess.check_call([sys.executable, str(SCRIPTS / "build.py")], cwd=ROOT)
    cfg = load_hou_config()
    if not cfg["api_key"]:
        raise SystemExit("No House API key. Set HOU_API_KEY or add scripts/.env")
    if cfg["house_id"] != DEFAULT_HOUSE_ID:
        raise SystemExit(
            f"Refusing unexpected house_id {cfg['house_id']} (expected BHG {DEFAULT_HOUSE_ID})"
        )
    print(f"auth -> {cfg['api_base']} house={cfg['house_id']}")
    token = exchange_token(cfg["api_base"], cfg["api_key"])
    summarize_sites(cfg["api_base"], token, cfg["house_id"])
    if args.list_only:
        return 0
    if not (SOURCE / "index.html").is_file():
        raise SystemExit("Missing source/index.html — run with --build")
    ...
    print("=== ensure Site ===")
    site = unwrap_site(ensure_site(cfg["api_base"], token, cfg["house_id"]))
    ...
    print(
        json.dumps(
            {
                "url": url,
                "site_slug": site.get("site_slug") or SITE_SLUG,
                "site_id": site.get("site_id") or site.get("id"),
                "library_path": LIBRARY_PREFIX + "/",
                "status": site.get("status"),
                "hostname": site.get("hostname") or site.get("host"),
            },
            indent=2,
        )
    )
